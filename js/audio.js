...
async function requestStt(formData) {
    if (location.protocol === "file:") {
        throw new Error("?�라?�드 ?�막?� file://?�서 ?�용?????�습?�다. ?�버 주소�??�어 주세??");
    }
    ...
    if (!contentType.includes('application/json')) {
        throw new Error("/api/stt ?�버 ?�수가 ?�행 중이 ?�닙?�다. Cloudflare Pages/Functions ?�는 배포 주소?�서 ?�어 주세??");
    }
    const data = await res.json();
    if (!res.ok) {
        const error = new Error(data.error || "?�라?�드 ?�막 ?�청 ?�패");
...
function getCloudRetryDelayMs(error) {
    ...
    const retryMatch = message.match(/(\d+(?:\.\d+)?)\s*�?s*??);
    if (retryMatch) return Math.ceil(Number(retryMatch[1]) * 1000) + 800;
    if (error?.status === 429 || message.includes("rate") || message.includes("?�청 ?�도 ?�한")) return 4500;
    return 0;
}
